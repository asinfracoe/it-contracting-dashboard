...
def get_sharepoint_client() -> Optional[SharePointClient]:
    ...
    def _get_token(self) -> str:
        ...
        logger.info("SharePoint: acquired access token")
        return self._token
    ...
    def _get_site_id(self) -> str:
        ...
        # Parse https://tenant.sharepoint.com/sites/SiteName
        without_scheme = self.site_url.replace("https://", "")
        ...
        logger.info("SharePoint: site_id=%s", self._site_id[:40])
        return self._site_id
    ...
    def _get_drive_id(self) -> str:
        if self._drive_id:
            return self._drive_id
        r = requests.get(
            f"https://graph.microsoft.com/v1.0/sites/{self._get_site_id()}/drive",
            headers=self._headers(), timeout=30,
        )
        r.raise_for_status()
        self._drive_id = r.json()["id"]
        logger.info("SharePoint: drive_id=%s", self._drive_id[:40])
        return self._drive_id
    ...
    def upload_file(
        ...
        """
        Upload *file_content* to <document library>/<folder_path>/<filename>.
        Returns the Graph DriveItem dict on success.
        Raises on any failure.
        """
        drive_id = self._get_drive_id()
        # URL-encode each segment individually
        safe_folder = requests.utils.quote(folder_path, safe="/")
        safe_file = requests.utils.quote(filename, safe="")
        upload_url = (
            f"https://graph.microsoft.com/v1.0"
            f"/drives/{drive_id}/root:/{safe_folder}/{safe_file}:/content"
        )
        content_type = mimetypes.guess_type(filename)[0] or "application/octet-stream"
        hdrs = {**self._headers(), "Content-Type": content_type}
        r = requests.put(upload_url, headers=hdrs, data=file_content, timeout=60)
        r.raise_for_status()
        item = r.json()
        logger.info(
            "SharePoint: uploaded '%s' → %s (size=%d)",
            filename, item.get("webUrl", ""), len(file_content),
        )
        return {
            "filename": filename,
            "folder": folder_path,
            "web_url": item.get("webUrl", ""),
            "size": item.get("size", len(file_content)),
            "id": item.get("id", ""),
        }
    ...
    def list_folder(self, folder_path: str = "BOMs") -> list:
        """List files in a SharePoint folder. Returns a list of file info dicts."""
        drive_id = self._get_drive_id()
        safe = requests.utils.quote(folder_path, safe="/")
        r = requests.get(
            f"https://graph.microsoft.com/v1.0/drives/{drive_id}/root:/{safe}:/children",
            headers=self._headers(), timeout=30,
        )
        if r.status_code == 404:
            return []
...
def get_sharepoint_client() -> Optional[SharePointClient]:
    """
    Returns a SharePointClient if credentials are configured, else None.
    The caller should check for None and fall back to mock/local behaviour.
    """
    global _client
    if _client:
        return _client
